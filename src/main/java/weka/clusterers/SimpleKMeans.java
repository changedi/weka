...
public class SimpleKMeans extends RandomizableClusterer implements
	...
	// add by zunyuan.jy
	private Instances m_ReInstances;
	private java.util.List<Integer> instanceResultList;
	private java.util.List<Double> instanceOriginClassList;
	...
	public void buildClusterer(Instances data) throws Exception {
		...
		// modify by zunyuan.jy
		instances.setClassIndex(1);
		instanceOriginClassList = new java.util.ArrayList<Double>();
		for (int i = 0; i < instances.numInstances(); i++) {
			instanceOriginClassList.add(instances.instance(i).classValue());
		}
		// end zunyuan.jy
		...
		while (!converged) {
			...
			// add by zunyuan.jy
			m_ReInstances = new Instances(instances, 0);
			instanceResultList = new java.util.ArrayList<Integer>();
			// end
			for (i = 0; i < instances.numInstances(); i++) {
				tempI[clusterAssignments[i]].add(instances.instance(i));
				// add by zunyuan.jy
				Instance inst = instances.instance(i);
				instanceResultList.add(clusterAssignments[i]);
				m_ReInstances.add(inst);
				// end
			}
			for (i = 0; i < m_NumClusters; i++) {
	...
	public String toString() {
			...
			for (int j = 0; j < m_NumClusters; j++) {
				if (m_ClusterCentroids.attribute(i).isNominal()) {
					if (m_ClusterCentroids.instance(j).isMissing(i)) {
						...
						valMeanMode = pad((strVal = m_ClusterCentroids
								.attribute(i).value(
										(int) m_ClusterCentroids.instance(j)
												.value(i))), " ", maxWidth + 1
								- strVal.length(), true);
					}
				} else {
					if (m_ClusterCentroids.instance(j).isMissing(i)) {
						...
						valMeanMode = pad((strVal = Utils.doubleToString(
								m_ClusterCentroids.instance(j).value(i),
								maxWidth, 4).trim()), " ", maxWidth + 1
								- strVal.length(), true);
					}
				}
				temp.append(valMeanMode);
						...
						if (m_ClusterCentroids.instance(j).isMissing(i)) {
							...
							stdDevVal = pad((strVal = plusMinus
									+ Utils.doubleToString(
											m_ClusterStdDevs.instance(j).value(
													i), maxWidth, 4).trim()),
									" ", maxWidth + 1 - strVal.length(), true);
						}
						temp.append(stdDevVal);
		...
		// add by zunyuan.jy
		for (int i = 0; i < m_ReInstances.numInstances(); i++) {
			temp.append("=====");
			temp.append(instanceOriginClassList.get(i));
			temp.append("::::");
			temp.append(m_ReInstances.instance(i).value(1));
			temp.append("::::");
			temp.append(instanceResultList.get(i));
			temp.append("\n");
		}
		// end
